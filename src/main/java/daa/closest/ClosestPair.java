...
public final class ClosestPair {
    ...
    // Returns squared distance of closest pair
    public static double closestDistance2(Point[] points, Metrics m) {
        ...
        Point[] py = Arrays.copyOf(px, px.length); // will maintain sorted-by-y via merge steps
        Arrays.sort(py, Comparator.comparingDouble(Point::y));
        Point[] aux = new Point[points.length]; // temp buffer for stable merges by y
        if (m != null) m.incAllocations(points.length * 3L);
        return divide(px, py, 0, px.length, aux, m);
    }

    // Work on px[l..r) sorted by x. py is the same set sorted by y. aux is buffer for merging.
